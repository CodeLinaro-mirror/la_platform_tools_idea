...
public abstract class DaemonAction {
  ...
  private static DaemonParameters daemonParameters6Dot3(BuildLayoutParameters layout) {
    try {
      Factory<PatternSet> patternSetFactory = PatternSets.getPatternSetFactory(PatternSpecFactory.INSTANCE);
      //noinspection JavaReflectionMemberAccess
      IdentityFileResolver fileResolver = IdentityFileResolver.class.getConstructor(Factory.class).newInstance(patternSetFactory);
      DefaultFileCollectionFactory collectionFactory = createCollectionFactory6Dot3(fileResolver, patternSetFactory);
      //noinspection JavaReflectionMemberAccess
      return DaemonParameters.class.getConstructor(BuildLayoutParameters.class, FileCollectionFactory.class).newInstance(
        layout, collectionFactory);
    }
  ...
  private static DefaultFileCollectionFactory createCollectionFactory6Dot3(IdentityFileResolver fileResolver,
    ...
    Class<?> taskDependencyFactoryClass = classLoader.loadClass("org.gradle.api.internal.tasks.TaskDependencyFactory");
    Class<?> defaultTaskDependencyFactoryClass = classLoader.loadClass("org.gradle.api.internal.tasks.DefaultTaskDependencyFactory");
    Object defaultTaskFactory = defaultTaskDependencyFactoryClass.getMethod("withNoAssociatedProject").invoke(null);
    //noinspection JavaReflectionMemberAccess
    Constructor<DefaultFileCollectionFactory> collectionFactoryConstructor = DefaultFileCollectionFactory.class.getConstructor(
      PathToFileResolver.class, taskDependencyFactoryClass, DirectoryFileTreeFactory.class, Factory.class, propertyHostClass,
      FileSystem.class);
    return collectionFactoryConstructor.newInstance(
      fileResolver, defaultTaskFactory, new DefaultDirectoryFileTreeFactory(), patternFactory, propertyHostNoOp, null);
  }
  ...
  private static DaemonParameters daemonParameters6Dot0(BuildLayoutParameters layout) {
    try {
      ...
      Object defaultTaskFactory = defaultTaskDependencyFactoryClass.getMethod("withNoAssociatedProject").invoke(null);
      //noinspection JavaReflectionMemberAccess
      Constructor<DefaultFileCollectionFactory> fileCollectionFactoryConstructor = DefaultFileCollectionFactory.class.getConstructor(
        PathToFileResolver.class, taskDependencyFactoryClass, DirectoryFileTreeFactory.class, Factory.class);
      DefaultFileCollectionFactory fileCollectionFactory = fileCollectionFactoryConstructor.newInstance(
        fileResolver, defaultTaskFactory, new DefaultDirectoryFileTreeFactory(), patternSetFactory);
      //noinspection JavaReflectionMemberAccess
      return DaemonParameters.class.getConstructor(BuildLayoutParameters.class, FileCollectionFactory.class).newInstance(
        layout, fileCollectionFactory);
    }
